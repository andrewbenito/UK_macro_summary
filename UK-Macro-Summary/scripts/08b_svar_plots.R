...
lapply(c('tidyverse','data.table','readxl', 'ggsci', 'here'),
...
variables <- c("10y Gilt yield", "FTSE-All", "2y Inflation Swap", "GBP Trade-weighted exchange rate")
...
input_data <- read_excel("inputs/input_data.xlsx")
colnames(input_data)[1] <- "date"
input_data <- input_data[c(2:(k+1), 1)]
input_data[1:n] <- input_data[1:n]*100
head(input_data)

hd <- read_excel("inputs/HistDecompCTM.xlsx", col_names = combos)
...
#for(i in 1:n){
  ...
  plot(change_plot)
  ggsave(gsub(" ", "-", paste("plots/", variables[i], "_change", label, ".png", sep="")), 
         plot=change_plot,
         width = 10, 
         height = 6, 
         dpi = 100, units = "in", device='png')
  ...
  cum_plot <- 
    ...
    geom_vline(xintercept = vline_date, lty=4) +
    scale_x_date(date_breaks = "10 days", date_labels = "%b-%d") +
    scale_y_continuous(n.breaks = 10) + 
  ...
  plot(cum_plot)
  ggsave(gsub(" ", "-", paste("plots/", variables[i], "_cum", label, ".png", sep="")), 
         plot=cum_plot,
         width = 10, 
         height = 6, 
         dpi = 100, units = "in", device='png')
