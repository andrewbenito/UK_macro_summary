...
setwd("//ad.eislercapital.com/ecap_network/Network/Common/MACRO MARKETS/Team/RL/tracking-GDP-forecasts/")

blpConnect()
source('functions/chartFormat.R')
...
for (start_year in years) {
  ...
  # US real GDP qoq
  fcast_series <- c('ECGDUS')
  outturn <- c("GDP CQOQ Index")
  series_name <- "US GDP (%qoq SAAR)"
  file_name <- "us"
  P=TRUE
  ...
  forecast.plot(l, P, series_name, y_label, file_name)
  ...
  # EZ real GDP QoQ
  fcast_series <- c('ECGQEU')
  outturn <- c("EUGNEMUQ Index")
  series_name <- "EA Real GDP Growth (%qoq)"
  file_name <- "ez"
  P=TRUE
  l <- forecast.prep(fcast_series, outturn, start_year, P)
  # Unpacked dataframes - can override forecasts
  ...
  fcast_series <- c('ECGQDE')
  outturn <- c("GRGDPPGQ Index")
  series_name <- "Germany Real GDP Growth (%qoq)"
  file_name <- "germany"
  P=TRUE
  l <- forecast.prep(fcast_series, outturn, start_year, P)
  # Unpacked dataframes - can override forecasts
  ...
  # France real GDP QoQ
  fcast_series <- c('ECGQFR')
  outturn <- c("FRGEGDPQ Index")
  series_name <- "France Real GDP Growth (%qoq)"
  file_name <- "france"
  P=TRUE
  l <- forecast.prep(fcast_series, outturn, start_year, P)
  # Unpacked dataframes - can override forecasts
  ...
  # Spain real GDP QoQ
  fcast_series <- c('ECGQES')
  outturn <- c("SPNAGDPQ Index")
  series_name <- "Spain Real GDP Growth (%qoq)"
  file_name <- "spain"
  P=TRUE
  l <- forecast.prep(fcast_series, outturn, start_year, P)
  # Unpacked dataframes - can override forecasts
  ...
  # Japan real GDP qoq
  fcast_series <- c('ECGDJP')
  outturn <- c("JGDPAGDP Index")
  series_name <- "Japan GDP (%qoq SAAR)"
  file_name <- "japan"
  P=TRUE
  l <- forecast.prep(fcast_series, outturn, start_year, P)
  # Unpacked dataframes - can override forecasts
  ...
  # China real GDP QoQ
  fcast_series <- c('ECGQCN')
  outturn <- c("CNGDPQOQ Index")
  series_name <- "China Real GDP Growth (%qoq)"
  file_name <- "china"
  P=FALSE
  l <- forecast.prep(fcast_series, outturn, start_year, P)
  # Unpacked dataframes - can override forecasts
  fcast <- l[[1]]
  outturns_p <- l[[2]]
  l <- list(fcast, outturns_p)
  forecast.plot(l, P, series_name, y_label, file_name)
  
  
  # New Zealand real GDP QoQ
  fcast_series <- c('ECGQNZ')
  outturn <- c("NZNTGDPC Index")
  series_name <- "New Zealand Real GDP Growth (%qoq)"
  file_name <- "new_zealand"
  P=FALSE
  l <- forecast.prep(fcast_series, outturn, start_year, P)
  # Unpacked dataframes - can override forecasts
  fcast <- l[[1]]
